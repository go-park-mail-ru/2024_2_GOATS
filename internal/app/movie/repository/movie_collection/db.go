...
func Obtain(ctx context.Context, db *sql.DB) (*sql.Rows, error) {
	...
	sqlStatement := `
	SELECT collections.id, collections.title, movies.id, movies.title, movies.card_url, movies.album_url, movies.rating, movies.movie_type, movies.release_date, countries.title FROM collections
